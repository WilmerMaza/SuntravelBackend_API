...
const { getCategorias, getCategoriasPK } = require("../Services/CategoriasService.js");
const router = Router();
...
router.get("/getCategoriasPK", async (req, res) => {
  try {
    const categoria = await getCategoriasPK();
    res.status(200).send(categoria);
  } catch (error) {
    const response = {
      isRegister: false,
